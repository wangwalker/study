...
A：这样的结果也很好理解，因为内存空间分配的粒度太大了，需要进一步细化内存空间的分配粒度。因为地址空间有三类进程信息：代码段、堆和栈，那么内存管理单元（Memory Management Unit，MMU）就可以使用**三对基址+界限寄存器**定位它们。这样，就将原来一大块地址空间分为三段。
...
A：OS通常会为每一个进程保存一个称为**页表（page table）**的数据结构，让虚拟页和物理页一一对应起来。通常每个进程内需要多个页，为了区分不同的虚拟页，需要将页分成两个组件：
