...
在上图中，当JavaScript引擎开始运行时，产生堆（heap）、栈（stack）和任务队列。堆中保存着所有对象资源，栈中主要保存着的所有在同步任务，"任务队列"中将加入各种事件（click，load，done）和宿主级别的任务。只要栈中的代码执行完毕，主线程就会去读取"任务队列"中的任务，在合适的时机再依次执行那些事件所对应的回调函数。
