...
@interface NSThreadViewController1 ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)downloadAvatars{
    [_users enumerateObjectsWithOptions:NSEnumerationConcurrent usingBlock:^(WRGithubUser * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj setIndex:@(idx)];
        [NSThread detachNewThreadSelector:@selector(downloadWithUser:)
                                 toTarget:self
                               withObject:obj];
    }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"github-user-cell-1"];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"github-user-cell-1"];
    }
